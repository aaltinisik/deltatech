...
class BusinessProcessSharingChatter(PortalChatter):
    def _check_business_project_access_and_get_token(
        self, res_model, res_id, token
    ):
        if not token or token == "null":
            business_project_sudo = BusinessProcesssPortal._document_check_access(
                self, res_model, res_id, token
            )
    ...
    def portal_chatter_init(
        ...
        token = self._check_business_project_access_and_get_token(
            res_model, res_id, kwargs.get("token")
        )
        if token:
            kwargs["token"] = token
    ...
    def portal_message_fetch(
        self, res_model, res_id, domain=False, limit=10, offset=0, **kwargs
    ):
        token = self._check_business_project_access_and_get_token(
            res_model, res_id, kwargs.get("token")
        )
        ...
        return super().portal_message_fetch(
            res_model, res_id, domain=domain, limit=limit, offset=offset, **kwargs
        )
