...
class StockPicking(models.Model):
    ...
    def create(self, vals_list):# this should restrict users from creating delivery/receipts manually, they should only be created from sale/purchase orders
        ...
        if 'move_ids_without_package' in vals: # client wanted the same check on validation to be done on saving too
            ...
            for move_data in vals['move_ids_without_package']: # we check if the lines were touched
                ...
                if len(move_data) > 2 and 'quantity' in move_data[2]:
                    move_id = move_data[1]
                    if isinstance(move_id, str) and "virtual" in move_id: # if the line contains virtual, it means it's a new line and we check it differently
                        if self.picking_type_code in ['incoming', 'outgoing']: # we check if the picking is incoming/outgoing, if yes we restrict creation
                            ...
                            if move_data[2]['quantity'] > move_data[2]['product_uom_qty']:
                                raise UserError(_("You can't add a line where the quantity done is greater than the quantity needed"))
